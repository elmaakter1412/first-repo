...
/*
- state : fun(n, target) -> returns 1 if it is possible to create a subset
            from numbers 1 to n ans from the sum "target"
- recurranse: fun(n, target) -> fun(n-1, target) OR
                                fun(n-1, target - nums[n])if
                                target >= nums[n]
